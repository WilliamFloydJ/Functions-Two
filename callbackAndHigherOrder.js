...
// CODE HERE
const contains = (arr, name, nameContains) => {
...
// CODE HERE
const uniq = (arr, remDuplicates) => {
  let uniqNames = [];
  for (let names of arr) {
    if (uniqNames.includes(names) === false) {
      uniqNames.push(names);
    }
  }
  remDuplicates(uniqNames);
};
...
// CODE HERE
const each = (nameArr, eachName) => {
  nameArr.forEach((name, nameNumb) => {
    eachName(name, nameNumb);
  });
};
/*
...
each(
  names,
  (eachName = (name, nameNumb) => {
    console.log(`The item at index ${nameNumb} is ${name}.`);
  })
);
////////// PROBLEM 7 //////////
...
// CODE HERE
const getUserById = (objArr, id, idFound) => {
  objArr.forEach((user) => {
    if (user.id === id) {
      idFound(user);
    }
...
// CODE HERE
const addingSeven = addingFactory(7);
console.log(addingSeven(7));
